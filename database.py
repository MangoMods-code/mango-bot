...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        """)
        # type can be: 'stock', 'api', or 'aegis'
        # aegis = Aegis Online reseller panel (HMAC-signed, returns keys + balance in one call)
        # api   = generic external API (configurable URL, method, headers, body)
        # stock = manually loaded keys
        await db.execute("""
        ...
        # Migrations: add any new columns to existing databases.
        migrations = [
            ("api_balance_url",  "TEXT"),
            ("api_balance_path", "TEXT DEFAULT 'balance'"),
            ("aegis_category",   "INTEGER"),
...
async def get_all_api_variants():
    """Return all API-type variants that have a balance URL configured (generic API only)."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute("""
            SELECT v.*, p.display_name as product_name
...
async def update_variant_aegis(variant_id, category, service):
    """Store the Aegis category + service IDs for this variant."""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
...
async def clear_all_keys():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        return count
                name         TEXT NOT NULL,
                price        INTEGER NOT NULL DEFAULT 1,
                type         TEXT NOT NULL DEFAULT 'stock' CHECK(type IN ('stock', 'api')),
                api_url      TEXT,
                api_method   TEXT DEFAULT 'GET' CHECK(api_method IN ('GET', 'POST')),
                api_headers  TEXT DEFAULT '{}',
                api_body     TEXT DEFAULT '{}',
                api_key_path TEXT DEFAULT 'key',
                enabled      INTEGER NOT NULL DEFAULT 1,
